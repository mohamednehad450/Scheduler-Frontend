...
const TrackInput: FC<TrackInputProps> = ({
  ...
  const { ref, active } = useMove(({ x }) => {
    ...
    if (!activeIndex.current) {
      const flat = value.flatMap(([x1, x2]) => [
        abs(x1 - adjustX),
        abs(x2 - adjustX),
      ]);
      let minIndex = 0;
    ...
    <div>
          ...
          <ActionIcon
            ...
            onClick={() => {
              ...
              if (last[1] + 0.05 < 0.9) {
                ...
                const newValue: Value[] = value.map((v) => [
                  v[0] * 0.89,
                  v[1] * 0.89,
                ]);
                newValue.push([0.9, 1]);
      ...
      >
        ...
        >
          ...
          >
            <PeriodTrack xScale={xScale} yScale={yScale} yValue={0} />
            {value.map((v, i) => (
              <PeriodMark
                key={i}
                item={v}
            ...
            {formatTooltip && activeIndex.current && active && (
              <>
                <rect
                  ...
                  x={`${xScale(
                    value[activeIndex.current[0]][activeIndex.current[1]]
                  )}%`}
                  width={"64"}
                  height={24}
