...
const ChangeSocketModal: FC<{ onDone: () => void, opened: boolean }> = ({ onDone, opened }) => {
    ...
    useEffect(() => { setSUrl(sContext?.url[0] || '') }, [opened])

    return (
        <Modal title={t("change-socket-url")} opened={opened} onClose={() => onDone()}>
            <form>
