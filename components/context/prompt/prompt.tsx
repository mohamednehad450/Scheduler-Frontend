...
interface PromptArgs {
  ...
  newSequence: [
    onDone: (newSeq?: Sequence) => void,
    initialSequence?: Partial<Sequence>
  ];
  newCron: [onDone: (newCron?: Cron) => void, initialCron?: Cron];
  newPin: [
    onDone: (newPin?: Pin) => void,
    usedPins: { [key: Pin["channel"]]: true },
    initialPin?: Pin
  ];
  linkSequence: [
    onDone: (seq?: Sequence) => void,
    sequenceId: Sequence["id"],
    initialIds?: Cron["id"][]
  ];
  linkCron: [
    onDone: (cron?: Cron) => void,
    cronId: Cron["id"],
    initialIds?: Sequence["id"][]
  ];
}
...
const usePromptContext = (): { context: PromptContext; modal: ReactNode } => {
  ...
  const [linkSequence, setLinkSequence] =
    useState<PromptArgs["linkSequence"]>();
